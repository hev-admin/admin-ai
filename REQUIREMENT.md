...
- [ ] 配置 CORS 允许域名，替换当前的 `cors()` 无限制配置
- [ ] 移除 JWT Secret 的弱默认值，启动时校验必要环境变量是否已设置
- [ ] 统一 bcrypt salt 轮数为 12
- [ ] 将 Token 黑名单和速率限制从内存存储迁移到 Redis（或提供可切换的适配器）
- [ ] 将已实现的 `permission.js` 中间件挂载到需要权限控制的路由上

### 阶段十二：后端数据验证

- [ ] 将已定义的 User Zod 验证器（`validators/user.js`）应用到 User 路由
- [ ] 为 Role 路由添加 Zod 验证器并应用
- [ ] 为 Menu 路由添加 Zod 验证器并应用
- [ ] 为 Setting 路由添加 Zod 验证器并应用
- [ ] 统一登录和注册的密码长度校验规则（当前登录允许 6 位，注册要求 8 位）

### 阶段十三：前端缺陷修复

- [ ] 在 auth store 中补充 `fetchUser()` 方法
- [ ] 为所有 Pinia store 添加 loading 和 error 状态管理
- [ ] 为用户管理页面添加表单验证规则（必填、格式校验）
- [ ] 为角色管理页面添加表单验证规则
- [ ] 为菜单管理页面添加表单验证规则
- [ ] 为设置管理页面添加表单验证规则
- [ ] 在路由守卫中增加基于权限的访问控制（不仅检查登录状态）

### 阶段十四：测试补全

- [ ] 后端：添加 Role 路由和服务的测试
- [ ] 后端：添加 Menu 路由和服务的测试
- [ ] 后端：添加 Log、Setting、Upload、Export 的测试
- [ ] 后端：添加 Rate Limit、Permission、Log 中间件的测试
- [ ] 前端：添加关键 Vue 组件的测试（系统管理页面）
- [ ] 为 vitest 配置覆盖率阈值，防止覆盖率下降

### 阶段十五：DevOps 基础设施

- [ ] 创建 GitHub Actions 工作流（lint + test + build）
- [ ] 将文档中的 Dockerfile 和 docker-compose.yml 提交为实际文件
- [ ] 创建 `.env.example` 文件，列出所有必要环境变量
- [ ] 添加后端健康检查接口（`/health`）

### 阶段十六：前端体验优化

- [ ] 完善 i18n：将页面中硬编码的中文字符串替换为 i18n 调用
- [ ] 为搜索输入添加防抖处理
- [ ] 表格添加空状态提示组件
- [ ] 日志页面增强：添加日期范围筛选、排序、详情查看
- [ ] 设置页面增强：从后端动态加载设置项，替代前端硬编码
- [ ] 后端 API 支持动态排序参数

### 阶段十七：工程化与性能（可选）

- [ ] 添加 Vite 构建压缩插件（gzip/brotli）
- [ ] 配置 bundle 分析工具（rollup-plugin-visualizer）
- [ ] 移动端响应式适配（侧边栏、弹窗、表格）
- [ ] 结构化日志输出（JSON 格式替代 console.error）
- [ ] 基础无障碍支持（ARIA 属性、键盘导航）
